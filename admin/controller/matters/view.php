...
class ControllerMattersView extends Controller {
    private $error = array();

    public function index() {
        $this->data = $this->load->language('matters/view');
        ...
        $this->data['matters'] = array();

    $matters = $this->model_billing_customer->getMatters($filter_data);

        foreach ($matters as $matter) {
            $this->data['matters'][] = array(
                'id'   => $matter['id'],
                'matterid'          => $matter['matterid'],
                'associate'         => $matter['associate'],
                'assigned_to'         => $matter['jobassignment'],
                'disbursementamount'=>$this->currency->format($matter['disbursementamount'], $this->config->get('config_currency')),

                'jobamount'        => $this->currency->format($matter['jobamount'], $this->config->get('config_currency')),

                'jobamountpayed'       => $this->currency->format($matter['jobamountpayed'], $this->config->get('config_currency')),

                'jobstatus'        => $matter['jobstatus'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),

                'date_added'    => date($this->language->get('datetime_format_short'), strtotime($matter['jobentrydate'])),

                'date_due' => date($this->language->get('datetime_format_short'), strtotime($matter['jobdate'])),
                'edit'          => $this->url->link('matters/view/form', 'token=' . $this->session->data['token'] . $url . '&id=' . $matter['id'], true),
                'view'         => $this->url->link('matters/view/view', 'token=' . $this->session->data['token'] . '&id=' . $matter['id'], true)
            );
        }
        ...
        $this->data['delete'] = $this->url->link('matters/view/delete', 'token=' . $this->session->data['token'], true);
        $this->data['insert'] = $this->url->link('matters/new_job/form', 'token=' . $this->session->data['token'], true);

         $this->data['view'] = $this->url->link('matters/view/form', 'token=' . $this->session->data['token'], true);


         $this->data['header'] = $this->load->controller('common/header');
        $this->data['footer'] = $this->load->controller('common/footer');

     $this->response->setOutput($this->render('matters/matters_list'));
   
        
    }
public function view(){
    $this->load->language("matters/view");
     $this->data['title'] = $this->language->get('heading_title');
     $this->data['text_transactions'] = $this->language->get('text_transactions');

        $this->data['header'] = $this->load->controller('common/header');
        $this->data['footer'] = $this->load->controller('common/footer');
        $matter_id = $this->request->get['id'];
        $this->data['jobs'] = $this->db->query("SELECT * FROM job WHERE id = '$matter_id' ")->rows;
    ...
    $this->data['system_company'] = $this->config->get('config_registered_name');
        ...
        $this->data['matter_id'] = $matter_id;
        $this->response->setOutput($this->render('matters/view'));
    ...
    public function form() {
        ...
        $this->response->setOutput($this->render('matters/new_job'));
    }
