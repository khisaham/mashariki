    ...
    <form method="post" action="<?php echo $delete; ?>" id="form-status">
      <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover">
          ...
          <tr>
            <td class="text-center"><?php if (in_array($status['status_id'], $selected)) { ?>
              <input type="checkbox" name="selected[]" value="<?php echo $status['status_id']; ?>" checked="checked" />
              <?php } else { ?>
              <input type="checkbox" name="selected[]" value="<?php echo $status['status_id']; ?>" />
              <?php } ?></td>
            <td class="text-left"><?php echo $status['name']; ?></td>
            <td class="text-right"><a href="<?php echo $status['edit']; ?>" title="<?php echo $button_edit; ?>" data-toggle="tooltip" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a></td>
          </tr>
          <?php } ?>
          <?php } else { ?>
          <tr>
